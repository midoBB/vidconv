...
def get_video_metadata_with_file(
    ...
    Returns:
        Tuple of (file_path, codec, width, height, framerate, bitrate_kbps, duration) or None
    """
    try:
...
def run_ffmpeg_hw(
    ...
    cmd = [
        ...
        "card",
        "-vf",
        f"hwdownload,format=nv12,scale=w={width}:h={height}:flags=lanczos:force_original_aspect_ratio=decrease,pad=ceil(iw/2)*2:ceil(ih/2)*2,unsharp=3:3:0.3:3:3:0.1,hwupload,scale_vaapi=format=nv12",
        "-r",
        str(framerate),
        "-movflags",
...
def run_ffmpeg_sw(
    ...
    cmd = [
        ...
        str(input_file),
        "-vf",
        f"scale=w={width}:h={height}:flags=lanczos:force_original_aspect_ratio=decrease,pad=ceil(iw/2)*2:ceil(ih/2)*2,unsharp=3:3:0.3:3:3:0.1",
        "-r",
        str(framerate),
        "-c:v",
        "libx264",
...
def process_file(
    ...
    # Skip condition check (already done during discovery for process_all, but keep for safety)
    if process_all:
